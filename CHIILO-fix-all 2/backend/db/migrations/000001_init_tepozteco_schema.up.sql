...
COMMENT ON COLUMN tepozteco.usuarios.perfil IS 'JSONB libre. Gov incluye: {organizacion, numTrabajador, dependencia, cargo, estado, fechaCreacion}. User incluye: {estado, fechaCreacion}';
...
COMMENT ON COLUMN tepozteco.analisis.resultado_json IS 'Estructura esperada: {nivel, confianza, zona, temp, humedad, viento, areas[], porcentaje_afectacion, modelo_version}';
...
COMMENT ON COLUMN tepozteco.reportes.parametros IS 'JSONB libre. Campos comunes: {zona, severidad:"Alta|Media|Baja", estado:"En Proceso|Resuelto|Cancelado"}';
...
COMMENT ON MATERIALIZED VIEW tepozteco.mv_dashboard_stats IS 'Refresca con: REFRESH MATERIALIZED VIEW CONCURRENTLY tepozteco.mv_dashboard_stats';
...
SELECT
    ...
    'admin@tepozteco.mx',
    '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TsCIMxMVpNMRFknQxMDkSnqDTVBa',
    r.id_rol,
...
SELECT
    i.id_imagen, i.uuid, i.nombre_archivo, u.nombre AS usuario, u.correo AS correo_usuario,
    LOWER(nr.clave) AS nivel_riesgo, nr.color_hex,
    ROUND((a.umbral_confianza * 100)::NUMERIC) AS confianza_pct, a.porcentaje_afectacion,
    a.resultado_json ->> 'zona' AS zona, i.resolucion_width || 'x' || i.resolucion_height AS resolucion,
    ROUND((i.tamano_bytes / 1048576.0)::NUMERIC, 1) AS tamano_mb, i.fecha_carga,
    a.id_analisis, a.fecha_analisis, ST_AsText(i.ubicacion_imagen) AS ubicacion_wkt,
...
SELECT
    r.id_reporte, r.tipo, r.contenido_summary, r.parametros ->> 'zona' AS zona,
    r.parametros ->> 'severidad'  AS severidad, r.parametros ->> 'estado' AS estado,
    u.nombre AS usuario, u.correo AS correo_usuario, LOWER(nr.clave) AS nivel_riesgo,
    nr.color_hex, a.umbral_confianza, a.resultado_json, r.fecha_generacion
...
SELECT
    b.id_log, b.tabla_nombre, b.operacion, b.registro_id, b.cambiado_por, b.descripcion,
    b.datos_antes, b.datos_despues, b.ip_origen, b.fecha, r.tipo AS reporte_tipo
...
COMMENT ON FUNCTION tepozteco.area_hectareas IS 'Convierte un polígono GEOGRAPHY a hectáreas.';